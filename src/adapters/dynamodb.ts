...
export class DynamoDBAdapter implements Adapter {
  ...
  async find(id: string): Promise<AdapterPayload | undefined> {
    ...
    >(await dynamoClient.send(new GetCommand(params))).Item;
  ...
  async findByUserCode(userCode: string): Promise<AdapterPayload | undefined> {
    ...
    const result = <
      { payload: AdapterPayload; expiresAt?: number } | undefined
    >(await dynamoClient.send(new QueryCommand(params))).Items?.[0];
  ...
  async revokeByGrantId(grantId: string): Promise<void> {
    ...
    do {
      ...
      const queryResult = await dynamoClient.send(new QueryCommand(params));
      ...
      await dynamoClient.send(new BatchWriteCommand(batchWriteParams));
